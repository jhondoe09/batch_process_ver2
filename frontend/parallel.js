...
// if (assignment_id && SubPid) {
//     const data = '';
//     const title = '';
//     const newUrl = 'https://172.16.2.113:4443/batch_process_ver2/parallel.html';
//     window.history.replaceState(data, title, newUrl);
// }
// FOR DEBUGGING
// const myFetchURL = 'https://172.16.2.113:4443/batch_process_ver2/backend/parallel.php';
// const fetchURL = 'https://172.16.2.113:8081/';
// FOR DEPLOYMENT
...
function addBatch() {
    ...
    let main_tbl_row =
        `<tr>
            <td>${line_number}</td>
            <td><input type="text" id="parts_number_${line_number}" class="form-control is-invalid" value="" onkeypress="return add_new_batch(${line_number}, event)" onclick="open_camera(${line_number})"></td>
...
function open_camera(line_number) {
    const inputField = document.getElementById(`parts_number_${line_number}`);
...
function confirm_identity() {
    ...
    const head_id = {
        229: "Divinia Soltones",
        492: "Maria Lorna Gamana",
        609: "Honeylet Haganas",
        480: "Jessel Ligumbras",
        2349: "Jean",
        357: "Liza",
        1180: "Elvey",
        2056: "Ronnel Saballa",
    ...
    if (main_id.value == '') {
        ...
        for (let key in head_id) {
            if (key == main_id.value) {
                swalALert('Good Job!', `Hi ${head_id[key]}!`, 'success');
                current_id.value = '';
                main_id.value = '';
                ...
                swalALert('Message Prompt!', 'Regrettably, your current permissions do not grant authorization to execute this action!', 'error');
            }
        }
    }
}
            ...
            if (response_data) {
                if (response_data.success) {
                    ...
                    if (response_data.data[0].operator_status === 'Started') {
                        ...
                        disableSaveBtn(response_data.data[0].operator_number);
                    } else if (response_data.data[0].operator_status === 'Updating') {
                        disableStartBtn(response_data.data[0].operator_number);
                        disableEndBtn(response_data.data[0].operator_number);
                        enableSaveBtn(response_data.data[0].operator_number);
                    }
                    else {
                ...
                .then(fetched_data => {
                        ...
                        if (fetched_data.success) {
                            ...
                            for (let data of fetched_data.data) {
                                ...
                                const tr =
                                    ...
                                    <td id="oninput_table_wafer_number_${data.line_number}_${data.wafer_number}_${operator_number}">${data.wafer_number}</td>
                    ...
                    myPromise.then(
                        function (value) {
                            ...
                            // getWaferNumber(parts_number, revision_number, lot_number, fetched_data.data[0].line_number, fetched_data.data[0].wafer_number, operator_number, value, operator_status);
                        },
                        function (error) {
                            disableSecondTable(error, operator_number, operator_status);
                            getNGWafer(result_type, operator_status, sequence_number, fetched_data.data[0].operator_number, fetched_data.data[0].line_number, limit, fetched_data.data[0].parts_number, fetched_data.data[0].revision_number, fetched_data.data[0].lot_number);
                            // getWaferNumber(parts_number, revision_number, lot_number, fetched_data.data[0].line_number, fetched_data.data[0].wafer_number, operator_number, error, operator_status);
                        }
                    );
                })
                .catch(error => {
...
function getWaferNumber(parts_number, revision_number, lot_number, line_number, wafer_number, operator_number, value, status) {
    ...
    if (value && sequence_number > 1 && status === 'Started') {
