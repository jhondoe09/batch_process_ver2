...
const hadan = urlParams.get('hadan');
// if (assign_id && sub_pid) {
//     const data = '';
//     const title = '';
//     const newUrl = 'https://172.16.2.137/batch_process_ver2/Standard.html';
//     window.history.replaceState(data, title, newUrl);
// }

// const fetchSubURL = 'http://172.16.2.137/tpc/GetSubProcess.php';
// FOR DEBUGGING
const myFetchURL = 'https://172.16.2.195:4443/batch_process_ver2/backend/query.php';
// FOR DEPLOYMENT
...
function getData() {
    ...
    fetchParams.append('item_parts_number', item_parts_number);
    fetchParams.append('revision_number', revision_number);
    fetchParams.append('lot_number', lot_number);
                ...
                `;
                    ...
                    addBatchBtn.addEventListener('click', () => {
                        ...
                        <tr id="second_table_tr_${data.operator_number}_${data.line_number}">
                            ...
                            <td><button type="button" class="btn btn-primary"  onclick="open_camera(${data.operator_number}, ${data.line_number})">Primary</button></td>
                        </tr>
                    `;
                        ...
                        document.getElementById(`wafer_no_from_${data.operator_number}_1`).value = localStorage.getItem('data_wafer_number_from');
                        document.getElementById(`wafer_no_to_${data.operator_number}_1`).value = localStorage.getItem('data_wafer_number_to');
                        document.getElementById(`qty_in_${data.operator_number}_1`).value = localStorage.getItem('data_qty_in');
...
function open_camera(operator_number, line_number) {
    document.getElementById('reader').innerHTML = '';
    document.getElementById('openCamera').click();
    document.getElementById('modal_operator_number').value = operator_number;
    document.getElementById('modal_line_number').value = line_number;
    const html5QrCode = new Html5Qrcode("reader");
    html5QrCode.clear();
    document.getElementById('cameraBtn').addEventListener('click', function () {

        const operator_number = document.getElementById('modal_operator_number').value;
        const line_number = document.getElementById('modal_line_number').value;
        const inputField = document.getElementById(`parts_number_${operator_number}_${line_number}`);
        const qrCodeSuccessCallback = (decodedText, decodedResult) => {
            ...
            document.getElementById('scannerClose').click();
            ...
            //     html5QrCode.clear();
            //     console.log(document.getElementById('reader').innerHTML);
            //     document.getElementById('reader').innerHTML = '';
            //     document.getElementById('scannerClose').click();
            //     inputField.dispatchEvent(enterEvent);
            // }).catch(err => {
                ...
                fetch(myFetchURL, {
                    ...
                    .then(add_batch_data => {
                        ...
                        if (add_batch_data.success) {
                            ...
                            // wafer_to_input.addEventListener('oninput', () => {
                            qty_in_input.value = wafer_to_input.value - wafer_from_input.value + 1;
                            // });
                        }
                        else {
...
function save_third_tbl_data(operator_number) {
    ...
    for (let j = 0; j < third_tbl_tr.length; j++) {
        ...
        save_third_tbl_data.append(`hadan`, hadan);
        save_third_tbl_data.append(`save_third_table_data`, 'true');
        for (const [key, value] of save_third_tbl_data) {
        ...
        fetch(myFetchURL, {
            ...
            .then(third_table_datas => {
                ...
                if (third_table_datas.success) {
                    // success_count++;
                    // if (success_count == third_tbl_tr.length) {
                    getData();
                    // }
                }
            })
...
function start_process(operator_number) {
    ...
    var id_number_input = document.getElementById(`operator_id_${operator_number}`);
    if (id_number == '') {
        swalALert('Message Prompt', 'Please do not leave ID Number field empty', 'info');
        id_number_input.classList.add('border-danger');
    } else {
        id_number_input.classList.remove('border-danger');
        ...
        fetch(myFetchURL, {
            ...
            .then(second_table_datas => {
                ...
                if (second_table_datas[0] != 0) {
                    let count = 0;
                    second_table_tbody.innerHTML = '';
                    if (localStorage.getItem('operator_status') == 'Started' || localStorage.getItem('operator_status') == 'Finished') {
                        ...
                        Swal.fire({
                            ...
                            if (result.dismiss === Swal.DismissReason.timer) {
                                ...
                                myPromise.then(
                                    function (value) { get_third_tbl(operator_number, accordion_count, batch_number, value); },
                                    function (error) { get_third_tbl(operator_number, accordion_count, batch_number, error); }
                                );
                            }
                        })
                    }
                    for (let data of second_table_datas) {
                        ...
                        const tr = `<tr id="second_table_tr_${data.operator_number}_${data.line_number}">
                                ...
                                <td><input id="wafer_no_from_${data.operator_number}_${data.line_number}" type="number" class="form-control bg-light" value="${data.wafer_number_from ? data.wafer_number_from : data.wafer_number_from ? data.wafer_number_from : ''}" style="width: ${data.wafer_number_from ? data.wafer_number_from.length + 5 : 10}ch"></td>
                                <td><input id="wafer_no_to_${data.operator_number}_${data.line_number}" type="number" class="form-control bg-light" onkeydown="onkeydown_wafer_no_to(${data.operator_number}, ${data.line_number})" value="${data.wafer_number_to ? data.wafer_number_to : data.wafer_number_to ? data.wafer_number_to : ''}" style="width: ${data.wafer_number_to ? data.wafer_number_to.length + 5 : 10}ch"></td>
                    ...
                    // if (count == second_table_datas.length) {

                    // }
                    //     console.log(`count: ${count}, data+COUNT: ${second_table_datas.length}`)
                    //     // let myPromise = new Promise(function (myResolve, myReject) {
                    //     //     let x = 0;
                    //     //     if (x == 0) {
                    //     //         myResolve(true);
...
function get_third_tbl(operator_number, accordion_count, batch_number, value) {
    if (value) {
        const second_table_tbody = document.getElementById(`second_table_${operator_number}`);
        const third_table_tbody = document.getElementById(`third_table_tbody_${operator_number}`);
            ...
            fetch(myFetchURL, {
                ...
                .then(third_table_datas => {
                    ...
                    if (third_table_datas[0] != 0) {
                        ...
                        for (let data of third_table_datas) {
                            ...
                            const tr =
                                `<tr id="third_table_tr_${data.line_number}_${data.wafer_number}_${operator_number}">
                                    ...
                                    <td contenteditable="${hadan == 1 && localStorage.getItem(`operator_status`) == 'Started' ? true : false}" oninput="oninput_table_wafer_number(${data.line_number}, ${data.wafer_number}, ${operator_number})" id="oninput_table_wafer_number_${data.line_number}_${data.wafer_number}_${operator_number}">${data.wafer_number}</td>
                        ...
                        promise.then(
                            function (value) { getWaferNumber(parts_number, revision_number, lot_number, third_table_datas[0].line_number, third_table_datas[0].wafer_number, operator_number, value, localStorage.getItem(`operator_status`)); },
                            function (error) { getWaferNumber(parts_number, revision_number, lot_number, third_table_datas[0].line_number, third_table_datas[0].wafer_number, operator_number, error, localStorage.getItem(`operator_status`)); }
                        );
                    }
                    else {
...
function getWaferNumber(parts_number, revision_number, lot_number, line_number, wafer_number, operator_number, value, status) {
    const fetchURL = 'https://172.16.2.13:8081/';
    const sequence_number = localStorage.getItem('sequence_number');
    const subPname = document.getElementById('subPname').innerText.trim();
    // var oninputEvent = new Event('input', {
    //     bubbles: true,
    // });

    if (value && sequence_number > 1 && status === 'Started' && subPname !== 'CWP RI Crack Check') {
        ...
        const url = new URL(fetchURL + 'get_wafer_number');
        ...
        fetch(url.toString(), {
            ...
            .then(datas => {
                console.log(datas);
                if (datas.success) {
                    var seconds = (datas.data.length);
                    Swal.fire({
                        title: 'Fetching wafer number!',
                        html: '<p class="lead">Fetching the latest operator data. Please be patient, as this may take a few moments.</p>',
                        timer: seconds,
                        timerProgressBar: true,
                        didOpen: () => {
                        ...
                        if (result.dismiss === Swal.DismissReason.timer) {
                            swalALert("Data Fetched", datas.message, "success");
                            let myPromise = new Promise(function (myResolve, myReject) {
                                let x = 0;
                                if (x == 0) {
                                    myResolve(true);
                            ...
                            myPromise.then(
                                function (value) {
                                    if (value) {
                                        ...
                                        for (let d of datas.data) {
                                            count++;
                                            const wafer_number_td = document.getElementById(`oninput_table_wafer_number_${line_number}_${count}_${operator_number}`);
                                            console.log(wafer_number_td);
                                            if (wafer_number_td) {
                                                // wafer_number_td.dispatchEvent(oninputEvent);
                                                wafer_number_td.textContent = d.wafer_number;
                                            }
                                        };
                                ...
                                function (error) { swalALert('Fetched!', error, 'error'); }
                            );
                        }
                    })
                ...
                if (qty_data.success) {
                    ...
                    for (let data of qty_data.data) {
                        if (data.quantity_out) {
                            ...
                            localStorage.setItem('data_wafer_number_from', data.wafer_number_from);
                            localStorage.setItem('data_wafer_number_to', data.wafer_number_to);
                            localStorage.setItem('data_qty_in', sum);
                        }
                    }
                }
            }
...
function oninput_table_wafer_number(line_number, wafer_number, operator_number) {
    console.log(line_number, wafer_number, operator_number);
    const wafer_from_input_td = document.getElementById(`oninput_table_wafer_number_${line_number}_${wafer_number}_${operator_number}`);
    const remarks_input = document.getElementById(`remarks_${line_number}_${wafer_number}_${operator_number}`);
    console.log(wafer_from_input_td.textContent);
    if (wafer_from_input_td.textContent == 0 || wafer_from_input_td.textContent == '') {
        remarks_input.value = '';
        wafer_from_input_td.textContent = wafer_number;
    } else {
        remarks_input.value = wafer_number;
    }
    // VP-AMS2R2J1K-PL|AMS2R2J1K-PL|R24E350-11~20|2024-06-25|9|3307
}